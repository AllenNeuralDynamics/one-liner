...
from one_liner.utils import Protocol, Encoding, RPCException, StreamException, DESERIALIZERS, _async_recv
...
class AsyncRouterClient:
    ...
    def __init__(self, protocol: Protocol = "tcp", interface: str = "localhost",
        ...
        self.rpc_client = AsyncZMQRPCClient(protocol=protocol, interface=interface,
                                            port=rpc_port, context=self._context)
        self.stream_client = AsyncZMQStreamClient(protocol=protocol,
                                             interface=interface,
                                             port=broadcast_port,
                                             context=self._context)

    async def call(self, obj_name: str, attr_name: str, args: list = None,
             kwargs: dict = None,
        ...
        """
        return await self.rpc_client.call(obj_name, attr_name, args, kwargs,
    ...
    async def get_stream(self, name: str) -> Tuple[float, Any]:
        ...
        """
        return await self.stream_client.get(name)

    async def enable_stream(self, name: str):
        ...
        return await self.rpc_client.call("__streamer", "enable", args=[name])

    async def disable_stream(self, name: str):
        ...
        """
        # Use rpc_client to enable/disable streams.
        return await self.rpc_client.call("__streamer", "disable", args=[name])

    async def get_stream_configurations(self, as_dict: bool = False) -> Streams | dict:
    ...
    async def server_version(self):
        ...
        return await self.rpc_client.call("__router_server", "version")[1]
...
class AsyncZMQRPCClient:

    __slots__ = ("context", "socket")

    def __init__(self, protocol: Protocol = "tcp", interface: str = "localhost",
                 port: str = "5555", context: zmq.asyncio.Context = None):
        self.context = context or zmq.asyncio.Context.instance()
        self.socket = self.context.socket(zmq.REQ)
        address = f"{protocol}://{interface}:{port}"
        self.socket.connect(address)

    async def call(self, obj_name: str, attr_name: str, args: list = None, kwargs: dict = None,
             deserializer: Encoding | Callable = "pickle") -> Tuple[float, Any]:
        ...
        await self.socket.send(pickle.dumps((obj_name, attr_name, args, kwargs)), copy=False)
        success, timestamp, data = await _async_recv(self.socket, deserializer=deserializer)
        if not success:
...
class AsyncZMQStreamClient:
    ...
    def __init__(self, protocol: Protocol = "tcp", interface: str = "localhost",
                 port: str = "5556", context: zmq.asyncio.Context = None):
        """
        """
    ...
    async def get(self, stream_name: str) -> Tuple[float, any]:
        ...
        """
        success, timestamp, data = await _async_recv(
                                            self.sub_sockets[stream_name],
                                            prefix=stream_name,
